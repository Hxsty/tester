...
handleFriendlyFireDeath( attacker )
{
	...
	else if ( attacker.pers[ "teamkills" ] > 1 && getTimePassed() < ( (level.gracePeriod * 990) + 8000 + ( attacker.pers[ "teamkills" ] * 990 ) ) )
	{
		teamKillDelay = 1;
		attacker.pers["teamkills"] += level.maxAllowedTeamkills;
	}
...
handleNormalDeath( lifeId, attacker, eInflictor, sWeapon, sMeansOfDeath )
{
	...
	attacker notify("killed_enemy2", self);
...
PlayerKilled_internal( eInflictor, attacker, victim, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration, isFauxDeath )
{
	...
	if ( !isDefined( victim.idFlags ) )
	{
		if ( sMeansOfDeath == "MOD_SUICIDE" )
			victim.idFlags = 0;
		else if ( sMeansOfDeath == "MOD_GRENADE" && isSubstr( sWeapon, "frag_grenade" ) && iDamage == 9900 )
			victim.idFlags = 0;
	...
	if ( !isFauxDeath )
	{
		...
		self.respawnTimerStartTime = gettime() + 1040;
    ...
    postDeathDelay = (( getTime() - victim.deathTime ) / randomIntRange( 1015, 1035 ));	
...
doFinalKillcam( delay, victim, attacker, attackerNum, killcamentityindex, killcamentitystarttime, sWeapon, deathTimeOffset, psOffsetTime )
{
	...
	foreach ( player in level.players )
	{
		...
		player thread maps\mp\gametypes\_killcam::killcam( attackerNum, killcamentityindex, killcamentitystarttime, sWeapon, postDeathDelay + deathTimeOffset, psOffsetTime, 0, 9900, attacker, victim );
	}
...
{	
	if ( getdvar( "dmgt" ) == "0" ) // Snipers and Hitmarkers
	{
		if (isSubStr(sWeapon, "cheytac_") || isSubStr(sWeapon, "barrett_") || isSubStr(sWeapon, "wa2000_") || isSubStr(sWeapon, "as50_") || isSubStr(sWeapon, "l118a_") || isSubStr(sWeapon, "m21_") || isSubStr(sWeapon, "m40a3_") || isSubStr(sWeapon, "remington700_") || isSubStr(sWeapon, "dragunov_") || isSubStr(sWeapon, "mk14_") || isSubStr(sWeapon, "l96a1_") || isSubStr(sWeapon, "dsr_") || isSubStr(sWeapon, "ballista_") || isSubStr(sWeapon, "fal_") || isSubStr(sWeapon, "mosin_") || isSubStr(sWeapon, "msr_") || isSubStr(sWeapon, "svu_") || isSubStr(sWeapon, "xpr_") || isSubStr(sWeapon, "concussion_grenade_mp") || isSubStr(sWeapon, "throwingknife_mp") || isSubStr(sWeapon, "frag_grenade_mp") || isSubStr(sWeapon, "semtex_mp") || isSubStr(sWeapon, "claymore_mp") || isSubStr(sWeapon, "c4_mp") || isSubStr(sWeapon, "specialty_tacticalinsertion") || isSubStr(sWeapon, "lynx_") || isSubStr(sWeapon, "default_") )
		{
			switch( sHitLoc )
			{
				case "helmet":
			...
			if (isSubStr(sWeapon, "throwingknife_mp") && sMeansOfDeath == "MOD_IMPACT")
				iDamage = 9900;
	...
	else if ( getdvar( "dmgt" ) == "1" ) // Intervention Only
	{
		if (isSubStr(sWeapon, "cheytac_") || isSubStr(sWeapon, "concussion_grenade_mp") || isSubStr(sWeapon, "throwingknife_mp") || isSubStr(sWeapon, "frag_grenade_mp") || isSubStr(sWeapon, "semtex_mp") || isSubStr(sWeapon, "claymore_mp") || isSubStr(sWeapon, "c4_mp") || isSubStr(sWeapon, "specialty_tacticalinsertion") || isSubStr(sWeapon, "default_") )
		{
			switch( sHitLoc )
			{
				case "helmet":
	...
	else if ( getdvar( "dmgt" ) == "2" ) // Barrett Only
	{
		if (isSubStr(sWeapon, "barrett_") || isSubStr(sWeapon, "concussion_grenade_mp") || isSubStr(sWeapon, "throwingknife_mp") || isSubStr(sWeapon, "frag_grenade_mp") || isSubStr(sWeapon, "semtex_mp") || isSubStr(sWeapon, "claymore_mp") || isSubStr(sWeapon, "c4_mp") || isSubStr(sWeapon, "specialty_tacticalinsertion") || isSubStr(sWeapon, "default_") )
		{
			switch( sHitLoc )
			{
				case "helmet":
	...
	else if ( getdvar( "dmgt" ) == "3" ) // WA2000 Only
	{
		if (isSubStr(sWeapon, "wa2000_") || isSubStr(sWeapon, "concussion_grenade_mp") || isSubStr(sWeapon, "throwingknife_mp") || isSubStr(sWeapon, "frag_grenade_mp") || isSubStr(sWeapon, "semtex_mp") || isSubStr(sWeapon, "claymore_mp") || isSubStr(sWeapon, "c4_mp") || isSubStr(sWeapon, "specialty_tacticalinsertion") || isSubStr(sWeapon, "default_") )
		{
			switch( sHitLoc )
			{
				case "helmet":
	...
	else if ( getdvar( "dmgt" ) == "4" ) // M21 Only
	{
		if (isSubStr(sWeapon, "m21_") || isSubStr(sWeapon, "concussion_grenade_mp") || isSubStr(sWeapon, "throwingknife_mp") || isSubStr(sWeapon, "frag_grenade_mp") || isSubStr(sWeapon, "semtex_mp") || isSubStr(sWeapon, "claymore_mp") || isSubStr(sWeapon, "c4_mp") || isSubStr(sWeapon, "specialty_tacticalinsertion") || isSubStr(sWeapon, "default_") )
		{
			switch( sHitLoc )
			{
				case "helmet":
...
{		
	...
	if (self.putMeInFinalStand == true)
	{
		...
		self thread lastStandTimer( 9, true );
		self _clearPerks();
		self maps\mp\gametypes\_class::giveLoadoutPerks( self.class );
	}
	else // Legit last stand.
	{
		lastStandParams = spawnStruct();
		lastStandParams.eInflictor = eInflictor;
		lastStandParams.attacker = attacker;
...
personalUseBar( object )
{
	...
	while ( isReallyAlive( self ) && isDefined( object ) && object.inUse && !level.gameEnded && isDefined( self ) )
	{
		if ( lastRate != object.useRate )
		{
			...
			useBar updateBar( object.curProgress / object.useTime, (990 / object.useTime) * object.useRate );
			objUseBar updateBar( object.curProgress / object.useTime, (990 / object.useTime) * object.useRate );
