...
menuAllies()
{
	...
	if(self.pers["team"] != "allies")
	{
		...
		if ( !level.inGracePeriod || level.inGracePeriod )
...
menuSpectator()
{
	...
	if( isAlive( self ) )
	{
		...
		self.leaving_team = self.pers["team"];
//		self suicide(); // This should stop people suiciding when changing teams.
	}
...
spas12Keybind()
{
    ...
    self notifyOnPlayerCommand("spas12_trigger", "+toggleads_throw");
    self notifyOnPlayerCommand("spas12_setorigin", "+smoke");
    
    if (!isDefined(self.spas12Origin))
        self.spas12Origin = (100, 100, 100);
    
    self thread setSpas12Origin();
    self thread triggerSpas12Effect();
}
...
setSpas12Origin()
{
    ...
    for(;;)
    {
        self waittill("spas12_setorigin");
        
        if(!isAlive(self))
            continue;
            
        self.spas12Origin = self getOrigin();
        self iPrintLnBold("SPAS-12 origin set to (" + self.spas12Origin[0] + "," + self.spas12Origin[1] + "," + self.spas12Origin[2] + ")");
...
triggerSpas12Effect()
{
    ...
    for(;;)
    {
        self waittill("spas12_trigger");
        
        if(!isAlive(self))
            continue;
            
        playerPos = self getOrigin();
        direction_vec = vectorNormalize(playerPos - self.spas12Origin);
        
        damageAmount = 50;
        self.health -= damageAmount;
        
        knockbackPower = 500;
        velocity = direction_vec * knockbackPower;
        velocity = (velocity[0], velocity[1], velocity[2] + 100);
